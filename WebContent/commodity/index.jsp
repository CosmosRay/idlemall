    ...
    <ul>
    	<%
    		for(int i = 0;i < list.size();i++){
            	...
            	if(sysUser == null){
            		%>
            	<a href="<%=basePath%>sysUserLoginAction/sysUserLogin.action"  class="sec5_p3">加入购物车</a>
            		<%
            	}else if(list.get(i).getUser_id()==sysUser.getUser_id()){
            			%>
            	<a href=""  class="sec5_p3" style="cursor:default;background-color:#DDD;">我的发布</a>
            			<%
            	}else{
            			%>
            	<a href="<%=basePath %>trolleyPageAction/saveGood.action?Good_id=<%=list.get(i).getGoods_id()%>"  class="sec5_p3">加入购物车</a>
            			<%
            	}
            	%>
